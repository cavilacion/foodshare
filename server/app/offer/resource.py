...
class OfferListResource(Resource):
    ...
    def post(self):
        ...
        if ecv.testing:
            ...
            p = Publisher()
            result = p.add_offer(host_id=host_id,portions=portions,price=price,info=info,time_ready=time_ready)
            return result, 201
...
class OfferResource(Resource):
    ...
    def put(self, offer_id):
        ...
        p = Publisher()
        result = p.update_offer(offer_id=offer_id, portions=portions, price=price, info=info, time_ready=time_ready)
        return result, 201

    def delete(self, offer_id):
        p = Publisher()
        result = p.update_offer(offer_id=offer_id)
        return result, 201
